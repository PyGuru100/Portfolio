...
class Ball(Turtle):
    def __init__(self, x_motion: float):
        super().__init__()
        self.hideturtle()
        self.penup()
        self.shape('circle')
        ...
        self.y_motion = 1.5 * x_motion

    def move(self):
        random_variance = random_choice((-1, 1)) * self.x_motion/10
        self.goto(self.xcor() + self.x_motion + random_variance,
                  self.ycor() + self.y_motion + random_variance)
...
class CollisionDetector:
    ...
    def collision_with_walls(self):
        ...
        if self.ball.ycor() >= (GAME_HEIGHT / 2 - SENSITIVITY):
            self.ball.y_motion *= -1
        if self.ball.ycor() <= -(GAME_HEIGHT / 2):
            self.paddle.alive = False
